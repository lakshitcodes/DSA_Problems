...
void lakshitcodes()
{
    ...
    f(i, 0, k)
    {
        a1 = min(a1, abs(arr[a].ff - arr[i].ff) + abs(arr[a].ss - arr[i].ss));
        b1 = min(b1, abs(arr[b].ff - arr[i].ff) + abs(arr[b].ss - arr[i].ss));
    }
    cout << min(a1 + b1, abs(arr[a].ff - arr[b].ff) + abs(arr[a].ss - arr[b].ss)) << endl;
}
