...
bool isPoss(int mid, um(int,int) &mp, int n, int m){
    int extra=0;
    int less=0;
    f(i,1,n+1){
        int count=0;
        ...
        // How many jobs are remaining for this worker after utilising 1 hour jobs
        less+=count-min(count,mid);
        // How many extra jobs can this worker contribute if assigned 2 hour jobs
        extra+=(mid-min(count,mid))/2;
    }
    return extra>=less;
}
void lakshitcodes(){
