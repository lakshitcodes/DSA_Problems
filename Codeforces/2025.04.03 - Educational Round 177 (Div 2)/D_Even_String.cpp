...
void solve(){
    ...
    int total=sum(count);
    vector<vi> dp(27,vi(total+1,0));
    dp[0][0]=1;

    for(int i=1;i<=26;i++){
        for(int j=0;j<=total/2;j++){
            dp[i][j]=dp[i-1][j];
            if(count[i]>0 && j-count[i]>=0){
                dp[i][j]=mod_add(dp[i][j],dp[i-1][j-count[i]],mod);
            }
        }
    }
    int ans=mod_mul(mod_mul(dp[26][total/2],factorial[total/2],mod),factorial[(total+1)/2],mod);
    for(int i=1;i<=26;i++){
        ans=mod_div(ans,factorial[count[i]],mod);
    }
    cout<<ans<<endl;
}
