...
bool isValid(int mid, vi &arr, m(int, vii) & mp)
{
    int n = arr.size();
    vi cost(n + 1, INT_MIN);
    cost[0] = arr[0];
    f(i, 0, n)
    {
        ...
        for (auto &j : mp[i])
        {
            if (j.ss <= mid && cost[i] >= j.ss)
            {
                cost[j.ff] = min(mid, cost[i] + arr[j.ff]);
            }
        }
    }
    if (cost[n - 1] >= 0)
...
void lakshitcodes()
{
    ...
    vi weights(weight.begin(), weight.end());
