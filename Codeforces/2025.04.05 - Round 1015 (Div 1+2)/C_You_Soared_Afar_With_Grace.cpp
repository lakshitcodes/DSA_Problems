...
void lakshitcodes() {
    ...
    vii moves;
    if (sameFound && sameIdx != n/2) {
        move(moves, a, b, idxA, idxB, sameIdx, n/2);
    }
 
    f(x,1,n+1) {
        int i = idxA[x], j = idxB[x];
        int correctJ = n-1-i;
        if (j == correctJ) continue;
        move(moves, a, b, idxA, idxB, j, correctJ);
    }
